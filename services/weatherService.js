...
const transformDayConditions = (marineData, forecastData) => {
    return marineData.time.map((time, index) => ({
        time,
    ...
    })).filter(condition => Object.values(condition).every(value => value !== null && value !== undefined));
};
...
const transformWeekConditions = (marineData, forecastData) => {
    return marineData.time.map((time, index) => ({
        ...
        wind_direction_10m_dominant: forecastData.wind_direction_10m_dominant[index],
    })).filter(condition => Object.values(condition).every(value => value !== null && value !== undefined));
};
